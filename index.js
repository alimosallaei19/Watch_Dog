...
client.on('messageCreate', async message => {
	...
	switch (args[0]) {
		case 'delete':
			// delete all channels in the category with id 957030606745788436
			console.log("running")
			await client.guilds.cache.get(process.env.SERVER_ID).channels.cache.get(process.env.END_CATEGORY).children.forEach(async c => {
				await c.delete();
			});
